'''Dataset for multiclass segmentation (MainDataset = traindataset, TestDataset = testdataset)'''
import os
import random
import numpy as np
...
class MainDataset(Dataset):
    ...
    def cv_random_flip(self, img, bg, solid, non_solid, p=0.5):
        if random.random() < p:
            img = img.transpose(Image.FLIP_LEFT_RIGHT)
            bg = bg.transpose(Image.FLIP_LEFT_RIGHT)
            solid = solid.transpose(Image.FLIP_LEFT_RIGHT)
            non_solid = non_solid.transpose(Image.FLIP_LEFT_RIGHT)
        return img, bg, solid, non_solid
        
    def randomRotation(self, img, bg, solid, non_solid, p=0.5, angle_range=(-15, 15)):
        if random.random() < p:
            angle = random.uniform(angle_range[0], angle_range[1])
            img = img.rotate(angle, Image.BICUBIC)
            bg = bg.rotate(angle, Image.NEAREST)
            solid = solid.rotate(angle, Image.NEAREST)
            non_solid = non_solid.rotate(angle, Image.NEAREST)

        return img, bg, solid, non_solid
    
    def randomCrop(self, img, bg, solid, non_solid, p=0.3):
        if random.random() < p:
            ...
            solid = solid.crop((left, top, left + crop_w, top + crop_h))
            non_solid = non_solid.crop((left, top, left + crop_w, top + crop_h))
            img = img.resize((width, height), Image.BICUBIC)
            bg = bg.resize((width, height), Image.NEAREST)
            solid = solid.resize((width, height), Image.NEAREST)
            non_solid = non_solid.resize((width, height), Image.NEAREST)
        return img, bg, solid, non_solid
    ...
    def apply_augmentation(self, img, bg, solid, non_solid):
        ...
        img, bg, solid, non_solid = self.cv_random_flip(img, bg, solid, non_solid, p=flip_p)

        img, bg, solid, non_solid = self.randomRotation(img, bg, solid, non_solid, p=rotate_p, angle_range=(-20, 20) if self.augment_intensity == 'heeavy' else (-15, 15))

        img, bg, solid, non_solid = self.randomCrop(img, bg, solid, non_solid, p = crop_p)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(imgs)):
            img = imgs[i]
            masks = all_masks[i]
            img, masks[0], masks[1], masks[2] = self.apply_augmentation(img, masks[0], masks[1], masks[2])
            imgs[i] = img
            all_masks[i] = masks
...
class DynamicDataset(Dataset):
    def __init__(self, root, trainsize, clip_len=3, max_num=None, augment_intensity='medium', seed=42, epoch=0):
        assert clip_len % 2 == 1, "clip_len must be odd"
        self.trainsize = trainsize
        self.augment_intensity = augment_intensity
        self.clip_len = clip_len
        ...
        for vid in sorted(os.listdir(root)):
            ...
            if self.max_num is not None and len(all_clips) > self.max_num:
                ...
                indices = random.sample(range(1, len(all_clips)), max_num)
